...
class Ui_MainWindow(object):
    ...
    def get_langs(self):
        self.ocr_languages_list.addItems(subprocess.run(['tesseract', '--list-langs'], capture_output=True, text=True).stdout.splitlines())
    ...
    def browseFiles(self):
        ...
        new_files, _ = QtWidgets.QFileDialog.getOpenFileNames(None, "Select Files to Merge", f"/Users/{self.username}/Desktop/", "PDF Files(*.pdf)",
    ...
    def mergePDFs(self):
        ...
        if self.file_list.count() > 1:  # no merging unless there are enough documents to merge
            ...
            output_file_name, _ = QtWidgets.QFileDialog.getSaveFileName(
                None, caption="Save File", directory=f"/Users/{self.username}/Desktop/merged", filter="PDF File (*.pdf)", options=options)
    ...
    def extractAnnotations(self):
        ...
        for i in range(self.file_list.count()):
            ...
            #cmd = f'pdfannots "{input_file}" -o "{output_file}"'
